/**
 * Read-only "Mounted content" section for a version (registry
 * specs/mount-lineage "History surfaces lineage"): one row per mount —
 * prefix, source origin, time-based version label, and a relative
 * retrieved-at — with a "version unavailable at commit time" badge when
 * resolution failed. The section is absent for versions without lineage
 * (pre-change versions, mountless workspaces): this component renders null.
 */
...
export function CommitMountedContent({ commitId }: { commitId: string }) {
  ...
  useEffect(() => {
    ...
    fetchCommitDetail(commitId)
      ...
      .catch(() => {
        // Old gateway / unknown version — behave like "no lineage": absent.
  ...
  if (rows === null) {
    return (
      <div className="flex items-center gap-1 px-1.5 py-1 text-xs text-muted-foreground">
        <Loader2 className="h-3 w-3 animate-spin" /> Loading version details…
      </div>
    );
  }
