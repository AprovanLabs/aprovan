...
function NotificationCard({ row, seen }: { row: NotificationRow; seen: boolean }) {
  ...
  return (
    <div className={`rounded-md border bg-card p-3 text-sm ${seen ? "opacity-60" : ""}`}>
...
export function NotificationsPanel({ scope: explicitScope }: NativePanelProps) {
  ...
  return (
    <PanelShell
      ...
      title="Notifications"
      description={
        scope ? `From ${scope.title ?? scope.name}` : "Decisions, warnings and activity"
      }
      actions={
        <>
          {scopeFilter}
    ...
    >
      ...
      {error ? (
        ...
        <PanelEmpty>
          {scope
            ? `${scope.title ?? scope.name} hasn't sent any notifications.`
            : filter === "all"
              ? "No notifications yet. Updates from apps and workflows appear here."
