 ...
 * conflict notifications, live workspace file sync, and the sync-state chip
 * subscription. File presence lives in `features/presence/` over the realtime
 * socket — not here.
 */
export function useDraftSync(args: {
  ...
  const { activeSession, setActiveSession, editSessionOpen, setSyncState } = args;
