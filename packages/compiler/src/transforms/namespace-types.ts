 ...
 *
 * The emitted file contains `declare module` blocks plus a global service
 * root (`declare const tools: …`) and no top-level import/export, so it stays
 * an ambient (script) declaration file. Configurable `rootFiles` in the
 * type environment are what make that global apply to project source.
 *
 * This module is the single implementation of that contract. It is published
 * as the dependency-free subpath `@aprovan/patchwork/namespace-types`
 ...
 *
 * Package-name / type-name derivation is shared with the registry catalog
 * type bundler via {@link toPascalCase}.
 */
...
export { toPascalCase } from "./identifier-case.js";
export {
  emitProviderModuleIndex,
  providerModuleName,
  resolveOnDemandProviderMounts,
} from "./provider-types-bundle.js";
export type {
...
export interface NamespaceTypesOptions {
   ...
   * Alias of {@link pluginDeclarations}; both are merged when present.
   */
  overrideTypes?: Record<string, string>;
  /**
   * Plugin-carried ambient declarations keyed by namespace — second input
   * alongside the gateway's namespace list. Merged with {@link overrideTypes}.
   */
  pluginDeclarations?: Record<string, string>;
  /**
   * Global binding name for the service root (default `tools`). Emitted as
   * `declare const <name>: { … }` so Stream 5 rootFiles globals resolve.
   */
  serviceRoot?: string;
}
...
function structuralDeclaration(namespace: string): string {
  const typeName = `${toPascalCase(namespace)}Namespace`;
  return `declare module ${JSON.stringify(namespace)} {
...
export function generateNamespaceTypes(
  ...
  // Plugin-carried declarations are a second input alongside the service list.
  const pluginTypes: Record<string, string> = {
    ...options.overrideTypes,
    ...options.pluginDeclarations,
  };
  for (const ns of Object.keys(pluginTypes)) {
    if (!namespaces.includes(ns) && IDENTIFIER.test(ns)) namespaces.push(ns);
  }

  const native = new Set<string>(NATIVE_APP_NAMESPACES);
  const blocks = namespaces.map((namespace) => {
    const pluginType = pluginTypes[namespace];
    if (pluginType) {
  ...
  const globalRoot =
    namespaces.length > 0
      ? `\ndeclare const ${serviceRoot}: {\n${rootMembers}\n};\n`
      : "";

  return `// Generated by @aprovan/patchwork — do not edit.
//
...
//   import app from "app";
//
// Each module is sugar over the namespace proxy the runtime installs as
// \`tools.<namespace>\`, so the import and the global behave identically.
