 ...
 */
function getParentBridge(proxy: Proxy, telemetry?: WidgetTelemetryHook): ParentBridge {
  if (!sharedBridge) {
    sharedBridge = new ParentBridge(proxy, telemetry);
  } else if (telemetry) {
    sharedBridge.setTelemetry(telemetry);
  }
  return sharedBridge;
}
...
export async function mountIframe(
  ...
  bridge.registerIframe(iframe, {
    ...
    traceId: `wt-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 8)}`,
  });
