...
export interface EditModalProps extends UseEditSessionOptions {
   ...
   *  LLM/model picker — its popovers open upward, so it belongs at the
   *  bottom of the screen, never in the header). */
  composerControls?: ReactNode;
  onClose: (finalCode: string, editCount: number) => void;
  onSave?: (code: string) => Promise<void>;
  onSaveProject?: (project: VirtualProject) => Promise<void>;
  /** Live runtime logs (console, errors, service calls) for the widget being
   *  edited — rendered as a collapsible Logs section under the preview. */
  logs?: EditorLogsSource;
...
export function EditModal({
  ...
  // Mobile-only slide-over visibility for the tree — always starts closed,
  // independent of `showTree`/`initialState`, so opening the editor never
  // covers the screen with the explorer on a small device (mirrors the chat
  // page's sidebar, which never auto-opens either). Toggled by its own
  // `md:hidden` header button; `showTree` still gates whether the tree
  // exists at all (the desktop "single file" toggle keeps working exactly
  // as before).
  ...
  const handleClose = useCallback(() => {
    ...
    // Anything unsaved counts — manual edits as much as AI edits. The
    // snapshot pair already tracks exactly "has the project changed since
    // the last save", so closing can never silently drop typed changes.
    const hasUnsavedChanges = projectSnapshot !== lastSavedSnapshot;
  ...
  }, [code, session, hasSaveHandler, onClose, projectSnapshot, lastSavedSnapshot]);
    ...
    <div className="fixed inset-0 z-50 flex flex-col bg-background">
        <div className="flex items-center gap-2 px-4 py-2 bg-background border-b shrink-0">
          ...
          <div className="ml-auto flex items-center gap-2">
            ...
            {!hideFileTree && (
              <>
                {/* Mobile: opens the tree as a slide-over (and ensures it
                    exists at all — mobile has no separate "single file"
                    state, only the drawer's open/closed). */}
                <button
                  onClick={() => {
                    setShowTree(true);
                    setMobileTreeOpen((open) => !open);
                  }}
                  className={`md:hidden px-2 py-1 text-xs rounded flex items-center gap-1 ${mobileTreeOpen ? 'bg-primary text-primary-foreground' : 'hover:bg-primary/20 text-primary'}`}
                  title="File tree"
                >
                  <FolderTree className="h-3 w-3" />
                </button>
                {/* Desktop: the existing persistent column vs. full-width
                    single-file toggle. */}
                <button
                  onClick={() => setShowTree(!showTree)}
                  className={`hidden md:flex px-2 py-1 text-xs rounded items-center gap-1 ${showTree ? 'bg-primary text-primary-foreground' : 'hover:bg-primary/20 text-primary'}`}
        ...
        {/* `relative` so the mobile drawer's overlay positions against this
            row (header + composer stay put) instead of the whole viewport. */}
        <div className="relative flex-1 min-h-0 border-b overflow-hidden flex">
          {!hideFileTree && showTree && (
            // Small screens: hidden by default behind the toggle above,
            // slides over the preview with a scrim, closes on file select —
            // the same MobileDrawer the chat page's sidebar uses. `md:relative
            // md:z-10` (baked into MobileDrawer) keeps the tree's context menu
            // (absolute, in the tree's shadow root) painting above the
            // preview column on desktop, where the drawer is a static column.
            <MobileDrawer
              open={mobileTreeOpen}
              onOpenChange={setMobileTreeOpen}
              widthClassName="w-64 max-w-[85vw] sm:max-w-none"
              // Opaque on mobile — it's an overlay above the preview, and a
              // translucent `bg-muted/20` (fine for the desktop column,
              // which sits *beside* the preview) let the content behind it
              // bleed through. `md:bg-muted/20` restores the original tint
              // once it's back in flow as a static column.
        ...
        <div className="p-4 border-t bg-primary/5 shrink-0 space-y-2">
          ...
          {composerControls && <div className="flex items-center">{composerControls}</div>}
