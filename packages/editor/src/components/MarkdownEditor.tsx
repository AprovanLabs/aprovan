...
export function MarkdownEditor({
  ...
  // Sync externally-set values into the editor. Two cases matter: the host
  // clearing the draft after send, and the host *prefilling* it (e.g. the
  // apps explorer's "create a workflow in chat" action). Only sync while the
  // editor is unfocused — while the user is typing, the editor itself is the
  // source of truth and `value` merely mirrors it, so writing back would
  // fight the cursor.
  useEffect(() => {
    if (!editor) return;
    if (value === editor.getText()) return;
    if (value === '') {
