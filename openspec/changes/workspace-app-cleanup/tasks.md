...
- [ ] 1.1 Rename root `package.json` `"name"` from `@aprovan/patchwork-workspace` to
...
- [ ] 1.2 Remove the dead `apps/**` glob from `pnpm-workspace.yaml`, keeping `client/**` and
      `packages/**`. Satisfies `specs/repo-identity` Requirement "Workspace package globs
      match the actual package layout".
- [ ] 1.3 Update `README.md`'s title (currently `# patchwork`) and opening description to
...
- [ ] 1.4 Run `pnpm install --frozen-lockfile` to confirm the rename doesn't require a
...
- [ ] 2.1 Create `client/web/src/contexts/patchwork-context.tsx`: move the
...
- [ ] 2.2 Create `client/web/src/contexts/shared-edit-session-context.tsx`: move
      `SharedEditSessionCtx` and `useSharedEditSession()` verbatim from L204–215.
- [ ] 2.3 Create `client/web/src/contexts/widget-error-reporter-context.tsx`: move the
...
- [ ] 2.4 Add a barrel `client/web/src/contexts/index.ts` re-exporting all three.

## 3. Tab routing & tab host

> Depends-on: 2 | Touches: client/web/src/features/tabs/** | Verify: `pnpm --filter @aprovan/patchwork-web build`

- [ ] 3.1 Create `client/web/src/features/tabs/tab-routing.ts`: move
...
- [ ] 3.2 Create `client/web/src/features/tabs/useTabs.ts`: a hook that owns `openTabs`,
...
- [ ] 3.3 Create `client/web/src/features/tabs/TabContent.tsx`: the render-dispatch IIFE
...
- [ ] 4.1 Create `client/web/src/features/widgets/useCompilerBootstrap.ts`: move the
...
- [ ] 4.2 Create `client/web/src/features/widgets/ChatArtifactBlock.tsx`: move verbatim from
      L252–290 (non-widget fenced-block renderer using `resolveRenderer`).
- [ ] 4.3 Create `client/web/src/features/widgets/ChatWorkflowPreview.tsx`: move
      `ChatWorkflowPreview` (L171–190), `workflowCustomPreview` (L192–196), and
      `loadWorkflowScript` (L201–202) verbatim.
- [ ] 4.4 Create `client/web/src/features/widgets/NotificationPathWidget.tsx`: move
      `NOTIFICATION_IMPORT_RE` and `NotificationPathWidget` verbatim from L799–845.
- [ ] 4.5 Create `client/web/src/features/widgets/createPreviewManifest.ts`: move
...
- [ ] 5.1 Create `client/web/src/features/self-heal/useWidgetSelfHeal.ts` matching the
...
- [ ] 5.2 Verify the budget and gating logic is copied byte-for-byte, not reimplemented —
...
- [ ] 6.1 Create `client/web/src/features/sidebar/useWorkspaceExplorer.ts`: move
...
- [ ] 6.2 Create `client/web/src/features/sidebar/WorkspaceSidebar.tsx`: move the
...
- [ ] 7.1 Create `client/web/src/features/sessions/useSessionOrchestration.ts` matching the
...
- [ ] 7.2 Create `client/web/src/features/sessions/useEditDraft.ts`: move `editSession`/
...
- [ ] 7.3 Create `client/web/src/features/sessions/useDraftSync.ts`: move the 20s
...
- [ ] 8.1 Create `client/web/src/features/chat/chat-transport.ts`: move the
      `DefaultChatTransport` setup (L1776–1805) and `editTransport` (L1722–1762) verbatim.
- [ ] 8.2 Create `client/web/src/features/chat/MessageParts.tsx`: move `ReasoningPart`
...
- [ ] 8.3 Create `client/web/src/features/chat/useChatSubmit.ts`: move `handleSubmit`
      ...
      prompt variable.
- [ ] 8.4 Create `client/web/src/features/chat/ChatDock.tsx`: move the chat/preview
...
- [ ] 9.1 Create `client/web/src/features/edit-modal/EditModalHost.tsx`: move the
...
- [ ] 10.1 Confirm (grep) that zero files under `client/web/src` import the bare
...
- [ ] 10.2 Add a short header comment to `client/web/src/components/ui/button.tsx` (or a
...
- [ ] 11.1 Rewrite `ChatPage.tsx` to import and compose every feature hook/component from
...
- [ ] 11.2 Wire context providers (`PatchworkCtx`, `SharedEditSessionCtx`,
      ...
      PanelHostProvider).
- [ ] 11.3 Confirm `client/web/src/pages/ChatPage.tsx` is ≤ 300 LOC (`wc -l`). Satisfies
      `specs/chat-app-structure` Requirement "Composition root is thin".
- [ ] 11.4 Confirm no new file created in work streams 2–9 exceeds ~500 LOC (`wc -l
...
- [ ] 12.1 Run `pnpm typecheck` from the repo root (turbo, covers all packages with a
      `check-types`/`typecheck` script) and `pnpm --filter @aprovan/patchwork-web build`
      (tsc + vite bundle) with zero errors.
- [ ] 12.2 Confirm none of `packages/bobbin/**`, `packages/mcp-app-server/**`,
...
- [ ] 12.3 Re-run the work stream 1 rebrand verify command and the work stream 10 grep
