...
- [ ] 1.1 Add `partitionAccess(path, callerSub, hiddenPrefixes)` and
...
- [ ] 1.2 Call `assertPartitionAccess` in `services.ts` vfs `read`, `write`, `delete` (after
...
- [ ] 1.3 Call the guard in `routes/fs.ts` `GET/PUT/DELETE /fs/:path` (spec scenario: HTTP plane
      404s; response shape identical to a nonexistent path).
- [ ] 1.4 Change both listing filters (`services.ts` vfs `list`, `routes/fs.ts` `GET /fs`) to
...
- [ ] 1.5 Extend the audited `apps.data` procedure to file partitions (`path?` arg per tech-plan
...
- [ ] 1.6 Integration tests: foreign read/write/delete 404 on both planes; hash-pinned read 404;
...
- [ ] 1.7 Update `registry/docs/app-data.md` "file plane forgets app data" section: hiding is now
...
- [ ] 2.1 Delete from `groups.ts`: `PrefixGrantRecord`, `addPrefixGrant`, `removePrefixGrant`,
...
- [ ] 2.2 Delete the three `/groups/:id/prefix-grants` routes and `prefixGrantSchema` from
      `routes/groups.ts`.
- [ ] 2.3 Remove the `GroupPrefixGrants` table from `db/schema.ts` and `infra/src/stack.ts`;
      update `tests/groups-dynamodb.test.ts` accordingly.
- [ ] 2.4 Remove the "Prefix grants" section, `PrefixGrant` type, and related state/handlers from
...
- [ ] 3.1 Implement `collectMountLineage(workspaceId)` in `vcs/mounts.ts` per the tech-plan
...
- [ ] 3.2 Extend `VcsSnapshot` with `mounts?` (sorted, inside the canonical identity hash) and
...
- [ ] 3.3 Tests: commit records git SHA and s3 manifest hash; identical tree + moved mount →
...
- [ ] 3.4 Update `registry/docs/vcs-and-sessions.md`: replace the "recording mount version tokens
...
- [ ] 4.1 Add `GET/POST/DELETE /groups/:id/profiles` routes (admin-gated) over WS-3 membership
      storage per the tech-plan API shape: idempotent attach, 404 on unknown profile (spec
      "Group capability is profile membership").
- [ ] 4.2 Rewire `authorize.ts` `mayInvokeTool`: admin → direct Permissions → WS-3
...
- [ ] 4.3 Delete `GroupToolGrants`: `groups.ts` functions (`addToolGrant`, `removeToolGrant`,
...
- [ ] 4.4 AdminPanel GroupsTab: add the Profiles section (list/attach/detach with target +
      credential label; empty states per ux.md), remove the Tool grants section.
- [ ] 4.5 Tests: attach/detach lifecycle; member of granted group authorized, detach revokes on
...
- [ ] 5.1 Update `NATIVE_SPECS` partition notes (`vfs`, `keyvalue`) in `capabilities.ts` to
...
- [ ] 5.2 Add `profile?: string` to `ProviderGrantCapability` and populate it from the WS-3
      profile resolution for each granted provider (spec group-profile-grants "Access pane names
      the executing profile").
- [ ] 5.3 Render the profile name in the Access tab's provider-grant rows in `app-detail.tsx`,
...
- [ ] 6.1 Chat file tree: render the caller's own `.personal/data/<sub>` listing entries as a
...
- [ ] 6.2 Commit/history detail: render the commit's `provenance` (+ snapshot `mounts` tokens)
