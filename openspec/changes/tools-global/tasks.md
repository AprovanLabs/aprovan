...
- [ ] 4.1 Implement `registerMiddleware` and `registerOverride` per the tech plan's interface block; `registerOverride` throws on duplicate namespace.
- [ ] 4.2 Wire the registry into `assembleTools()` so overrides receive the node they shadow (wrap-with-delegate) and may provide namespaces absent from the gateway list.
- [ ] 4.3 Accept an optional `types` declaration string per override and expose it to the type generator.
- [ ] 4.4 Convert `widgetTelemetrySdk` in `client/web/src/lib/telemetry.ts` from an ad-hoc facade into a registered `telemetry` override that delegates to `telemetry.export`.
- [ ] 4.5 Convert the notification payload binding into a plugin-provided `notification` namespace; delete `NOTIFICATION_IMPORT_RE` and the source-rewriting in `NotificationPathWidget.tsx`.
- [ ] 4.6 Add tests for the `namespace-plugins` scenarios: chained middleware, delegate-receiving override, plugin-provided namespace, duplicate-registration error, and that sandboxed code cannot register.
...
- [ ] 5.1 Implement the `tools.`-access scanner returning `{ namespaces, unresolved }`; retarget `parseScriptDependencies` in `registry/packages/runtime/src/imports.ts` from import specifiers to `tools.` accesses.
- [ ] 5.2 Handle shadowing — a local binding named `tools` in an inner scope must not be attributed as a service access.
- [ ] 5.3 Set `unresolved: true` for computed access (`tools[expr]`) and surface it to consumers rather than reporting a complete list.
- [ ] 5.4 Delete `parseUsesAttribute` and `WidgetDependency` and their re-exports; remove `uses` threading from `MessageParts.tsx` and `ChatWidgetArtifact.tsx`.
- [ ] 5.5 Repoint the dependency panel to the scanner's output, rendering the incomplete-list indicator when `unresolved`.
- [ ] 5.6 Add tests for the five `widget-dependency-scan` scenarios.
...
- [ ] 7.1 Rename `@aprovan/patchwork-compiler` → `@aprovan/patchwork` and update every importer. Land as its own commit.
- [ ] 7.2 Rename `@aprovan/patchwork-editor` → `@aprovan/editor` and update every importer. Land as its own commit.
- [ ] 7.3 Update the esm.sh specifier and version constant in `server/workspace/src/routes/live-apps.ts` and the duplicate image pin in `client/web/src/features/widgets/useCompilerBootstrap.ts`.
- [ ] 7.4 Leave all `patchwork:*` and `patchwork_access_token` localStorage keys unchanged; add a comment recording that this is deliberate.
- [ ] 7.5 Update the publish workflow's package list.
...
- [ ] 8.1 Rewrite the `APP_SHELL_COMPILER_VERSION` comment block (lines 64-79): the hazard it documents — `vfs`, `events`, and `agents` resolving to unrelated npm packages — no longer exists once no bare specifier is claimed.
- [ ] 8.2 Update `live-apps.test.ts:343`, which asserts that bare `keyvalue`/`vfs` specifiers must not reach esm.sh, to assert the new invariant instead.
- [ ] 8.3 Decide and document whether the exact pin is still warranted for reproducibility alone, now that correctness no longer depends on it.
