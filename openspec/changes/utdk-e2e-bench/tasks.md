...
- [ ] 1.1 Add `@aws-sdk/client-ssm` as a direct dependency of `@utdk/e2e` (`package.json`).
- [ ] 1.2 Create `src/ssm-env.ts` exporting `fetchFromSsm(names, options?)` per the tech-plan's
...
- [ ] 1.3 Extend `scripts/write-env.ts` with `--from-ssm` and `--force`: derive the same
      ...
      refresh".
- [ ] 1.4 Add `tests/ssm-env.test.ts` using a mocked `SSMClient` (e.g. `aws-sdk-client-mock`) to
...
- [ ] 1.5 Update the README's "Credentials" section to document `--from-ssm`/`--force` alongside
...
- [ ] 2.1 Create `.github/workflows/utdk-e2e-nightly.yml`: `permissions: {contents: read,
...
- [ ] 2.2 Steps: checkout → setup pnpm/node → `pnpm install --frozen-lockfile` →
...
- [ ] 2.3 Add a reporting step using `actions/github-script`: parse `live-results.json`
...
- [ ] 2.4 Add a header comment (matching the style of `registry-deploy.yml`/`web.yml`) stating
...
- [ ] 2.5 Manual verification (not automatable from this task list): `gh workflow run
      ...
      trigger (tech-plan Rollout step 2).
