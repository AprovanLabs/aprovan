...
- [x] 5.1 Reserve the `svc#` scope namespace: reject caller-supplied `svc#` scopes at
...
- [x] 5.2 Chat sessions + transcripts onto records: session records under
...
- [x] 5.3 VCS refs/commits/snapshots onto records (snapshots use the existing >350KB
      S3 spill); mounts onto `svc#vcs#mounts` behind the stream-3 cache.
- [x] 5.4 One-shot migration sweep in `scripts/migrate-services-to-records.ts`
      (per-subsystem flags): read legacy `.services` files → write records → delete
      files; chat/vcs subsystems wired here.
- [x] 5.5 Tests: append cost O(appended) (write-count spy on a 500-message session),
...
- [x] 6.1 Apps (registrations + installs), agents (definitions + runs), sandboxes
      (registrations/hosts/runs/defaults) onto their `svc#` scopes.
- [x] 6.2 Workflows (registrations + cron cursors), webhooks (registrations + HMAC
      secrets), sync state, events append-logs (record-per-entry, seq keys), LLM
      jobs, workspace settings onto their `svc#` scopes.
- [x] 6.3 Legacy `.services/keyvalue/*` sweep into existing record scopes;
      `.services/bindings` gets a tombstone read path only (dies with WS-3 Profiles —
      do not build it a record home).
- [x] 6.4 Extend the stream-5 migration script to these subsystems; assert via test
...
- [x] 7.5 `FsStoreDsql` (metadata in DSQL, blobs/presigned flow unchanged on S3;
      ...
      (tech-plan §5).
- [x] 7.6 Run the existing FS/record/credential/audit contract suites against the DSQL
...
- [x] 8.1 Define `IIdentityStore` + `getIdentityStore()` (tech-plan §4); Dynamo
...
- [x] 8.2 Relational schema (SQLite + DSQL): users/workspaces/memberships/
...
- [x] 8.3 Wire the stream-3 principal cache invalidation into the identity store's
      mutation methods (single choke point instead of per-module hooks).
- [x] 8.4 Grep-gate test: no `dynamo()` usage for identity entities outside
...
- [x] 9.1 `snapshot-to-sqlite.ts`: Dynamo latest pointers + S3 blobs + records/
      ...
      mirror").
- [x] 9.2 `verify-snapshot.ts`: boot `WORKSPACE_MODE=local` against the mirror; smoke
...
- [x] 9.3 `reseed-dsql.ts`: idempotent chunked reseed (≤3,000 rows/txn) of latest FS
...
- [x] 9.4 Write `cutover-runbook.md` (read-only flag → snapshot → verify → reseed →
