...
- [x] 1.1 `rm -rf registry/infra/cdk.out` (6.7 GB gitignored build artifact; not git-tracked, local
      cleanup only)
- [x] 1.2 Delete `registry/packages/fn` (`@utdk/fn`, 945 LOC, fully orphaned — confirmed zero
      importers of `@utdk/fn` across all three repos)
- [x] 1.3 Delete `registry/apps/tailor` (`@aprovan/tailor`, 1,456 LOC, duplicated into
...
- [x] 1.4 Delete `registry/experiments/` (1,610 LOC, last touched 2026-03, no importers)
- [x] 1.5 Delete `registry/packages/utdk-isolate` (`@utdk/isolate` package) in its entirety
- [x] 1.6 In `registry/apps/workspace/src/isolate.ts`: remove `tryLoadIsolate`, the
...
- [x] 1.7 Update the module-level doc comment at the top of `isolate.ts` (currently describes the
      "When `packages/utdk-isolate` (APR-15) is available..." two-path design) to describe the
      single-path design
- [x] 1.8 Delete `registry/temp.md`, `registry/tasks.md`, `registry/utcp_config.json`,
      `registry/uv.toml`, `registry/.python-version`, `registry/.registry/`
- [x] 1.9 Grep the repo for stragglers: `grep -rn '@utdk/fn\|@utdk/isolate\|utdk-isolate\|apps/tailor' registry --include=*.ts --include=*.tsx --include=*.json | grep -v node_modules` must return nothing outside `.turbo` cache dirs
...
- [x] 2.1 Delete `packages/bobbin` (`@aprovan/bobbin`, 5,971 LOC) in its entirety
- [x] 2.2 In `packages/editor/src/components/edit/EditModal.tsx`: remove the `Bobbin` import and
...
- [x] 2.3 Remove `@aprovan/bobbin` from `packages/editor/package.json` and
      `client/web/package.json` dependencies, and from `packages/editor/tsup.config.ts` if
      referenced there
- [x] 2.4 Delete `packages/mcp-app-server` (`@aprovan/patchwork-mcp`, 4,112 LOC) in its entirety
- [x] 2.5 Delete `packages/patchwork` in its entirety (`@aprovan/patchwork`) — confirmed its only
...
- [x] 2.6 Remove `@aprovan/patchwork` from `client/web/package.json` dependencies
- [x] 2.7 Delete `packages/compiler/src/vfs/store.ts`, `packages/compiler/src/vfs/backends/http.ts`,
...
- [x] 2.8 Update `packages/compiler/src/vfs/index.ts` to drop the re-exports of `VFSStore`,
...
- [x] 2.9 Update `packages/compiler/src/index.ts`'s `// VFS` export block (currently exports
...
- [x] 2.10 Trim `packages/compiler/src/__tests__/vfs-core.test.ts`: remove the `describe("vfs/sync/differ")`
...
- [x] 2.11 In `packages/editor/src/lib/vfs.ts`: delete `getVFSStore`, `saveProject`, `loadProject`,
...
- [x] 2.12 In `packages/editor/src/components/CodePreview.tsx`: remove the `import {
...
- [x] 2.13 Update `packages/editor/src/index.ts`: remove the `getVFSConfig, getVFSStore,
...
- [x] 2.14 In `packages/editor/src/components/ServicesInspector.tsx`: delete the `ServicesInspector`
...
- [x] 4.2 **BLOCKED by 4.1 npm auth** `npm deprecate @aprovan/bobbin "Deleted in purge-dead-code (WS-1); the visual-edit panel is discontinued. See git history for the source."`
- [x] 4.3 **BLOCKED by 4.1 npm auth** `npm deprecate @aprovan/patchwork-mcp "Deleted in purge-dead-code (WS-1); MCP-Apps distribution is rebuild-later-if-ever. See git history for the source."`
- [x] 4.4 **BLOCKED by 4.1 npm auth** `npm deprecate @aprovan/patchwork "Deleted in purge-dead-code (WS-1); its only consumer (@aprovan/patchwork-mcp) was removed. See git history for the source."`
- [x] 4.5 **BLOCKED by 4.1 npm auth** Confirm npm publish auth for the `@utdk` scope is available; if not, stop here and leave
      4.6–4.7 unchecked
- [x] 4.6 **BLOCKED by 4.1 npm auth** `npm deprecate @utdk/fn "Deleted in purge-dead-code (WS-1); fully orphaned. See git history in the registry repo."`
- [x] 4.7 **BLOCKED by 4.1 npm auth** `npm deprecate @utdk/isolate "Deleted in purge-dead-code (WS-1); the gateway's direct in-process executor is now the sole execution path (registry/apps/workspace/src/isolate.ts). See git history in the registry repo."`
- [x] 4.8 **BLOCKED by 4.1 npm auth** Verify none of the above were unpublished: `npm view <package> versions --json` still
