...
All published packages live under the `@aprovan/` npm scope (GitHub Packages) and are prefixed by their **project name** (`registry-`, …). Packages in the `core` repo drop the project prefix — core is the umbrella (`@aprovan/ui`, `@aprovan/node`, `@aprovan/devtools`).
