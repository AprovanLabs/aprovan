...
export default async function run() {
  const listing = await tools.keyvalue.list({ prefix: "task:" });
  const queue = [];
  ...
  for (const entry of queue.slice(0, 2)) {
    ...
    if (agent.agent) {
      try {
        const resolved = await tools.agents.get({ name: agent.agent });
    ...
    const llm = tools[providerId];
    if (!llm || !llm.createChatCompletion) {
      results.push({ id: task.id, skipped: "provider " + providerId + " not available" });
      continue;
    }

    const created = await tools.sessions.create({ title: "Agent: " + task.title, mode: "staged" });
    const session = created.session;
    task.status = "in_progress";
    task.sessionId = session.id;
    task.updatedAt = new Date().toISOString();
    await tools.keyvalue.set({ key: entry.key, value: task });
    ...
    for (let round = 0; round < 2; round++) {
      ...
      await tools.vfs.write({ path: outputPath, content: summary + "\n", session: session.id });
    ...
    await tools.notifications.emit({
      category: "decision",
